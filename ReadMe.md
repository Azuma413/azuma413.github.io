...
```js
{
  ...
  image: '/images/news/xxx.jpg', // 任意。行をクリックで写真をフルスクリーン表示
}
```

一覧はサムネイルを置かず**日付＋タイトルのみ**（小さすぎて見えないため）。
代わりに、`href` があれば → タイトルからリンク（行末に `›`）、
`image` だけあれば → クリックでライトボックス表示（行末に写真アイコン）。
両方ある場合は `href` が優先されるので、写真も見せたいときはリンク先のページに載せる。
